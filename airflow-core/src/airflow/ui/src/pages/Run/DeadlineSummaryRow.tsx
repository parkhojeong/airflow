...
type DeadlineSummaryRowProps = {
  readonly alert?: DeadlineAlertResponse;
  readonly deadline: DeadlineResponse;
  readonly runEndDate?: string;
};

export const DeadlineSummaryRow = ({ alert, deadline, runEndDate }: DeadlineSummaryRowProps) => {
  const { t: translate } = useTranslation("dag");
  const deadlineTime = dayjs(deadline.deadline_time);

  let actualDurationLabel: string | undefined;

  if (deadline.missed && runEndDate !== undefined) {
